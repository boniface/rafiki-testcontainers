...
import resolvable, { type Resolvable } from '@apollo/server/src/utils/resolvable.js';
import {
  GraphQLError,
  assertValidSchema,
  print,
...
import { newCachePolicy } from '@apollo/server/src/cachePolicy.js';
import { determineApolloConfig } from '@apollo/server/src/determineApolloConfig.js';
import {
  ensureError,
  ensureGraphQLError,
  normalizeAndFormatErrors,
} from '@apollo/server/src/errorNormalize.js';
import {
  ApolloServerErrorCode,
  ApolloServerValidationErrorCode,
} from '@apollo/server/src/errors';
import type { ApolloServerOptionsWithStaticSchema } from '@apollo/server/src/externalTypes/constructor.js';
import type {
  ExecuteOperationOptions,
  VariableValues,
} from '@apollo/server/src/externalTypes/graphql.js';
import type {
  ApolloConfig,
  ApolloServerOptions,
  ApolloServerPlugin,
...
} from '@apollo/server/src/externalTypes';
import { runPotentiallyBatchedHttpQuery } from '@apollo/server/src/httpBatching.js';
import type { GraphQLExperimentalIncrementalExecutionResults } from '@apollo/server/src/incrementalDeliveryPolyfill.js';
import { pluginIsInternal, type InternalPluginId } from '@apollo/server/src/internalPlugin.js';
import {
  preventCsrf,
  recommendedCsrfPreventionRequestHeaders,
} from '@apollo/server/src/preventCsrf.js';
import { APQ_CACHE_PREFIX, processGraphQLRequest } from '@apollo/server/src/requestPipeline.js';
import { newHTTPGraphQLHead, prettyJSONStringify } from '@apollo/server/src/runHttpQuery.js';
import { HeaderMap } from '@apollo/server/src/utils/HeaderMap.js';
import { UnreachableCaseError } from '@apollo/server/src/utils/UnreachableCaseError.js';
import { computeCoreSchemaHash } from '@apollo/server/src/utils/computeCoreSchemaHash.js';
import { isDefined } from '@apollo/server/src/utils/isDefined.js';
import { SchemaManager } from '@apollo/server/src/utils/schemaManager.js';
...
export class ApolloServer<in out TContext extends BaseContext = BaseContext> {
  ...
  private async addDefaultPlugins() {
    ...
    // Special case: cache control is on unless you explicitly disable it.
    {
      if (!alreadyHavePluginWithInternalId('CacheControl')) {
        const { ApolloServerPluginCacheControl } = await import(
          '@apollo/server/src/plugin/cacheControl'
        );
      ...
      if (!alreadyHavePlugin && apolloConfig.key) {
        if (apolloConfig.graphRef) {
          ...
          const { ApolloServerPluginUsageReporting } = await import(
            '@apollo/server/src/plugin/usageReporting'
          );
          plugins.unshift(
      ...
      if (!alreadyHavePlugin && enabledViaEnvVar) {
        if (apolloConfig.key) {
          const { ApolloServerPluginSchemaReporting } = await import(
            '@apollo/server/src/plugin/schemaReporting'
          );
    ...
    // Special case: inline tracing is on by default for federated schemas.
    {
      ...
      if (!alreadyHavePlugin) {
        ...
        const { ApolloServerPluginInlineTrace } = await import(
          '@apollo/server/src/plugin/inlineTrace'
        );
        plugins.push(
    ...
    if (!alreadyHavePlugin) {
      ...
      } = await import('@apollo/server/src/plugin/landingPage/default');
...

    {
      ...
      if (hideSchemaDetailsFromClientErrors && !alreadyHavePlugin) {
        const { ApolloServerPluginDisableSuggestions } = await import(
          '@apollo/server/src/plugin/disableSuggestions'
        );
