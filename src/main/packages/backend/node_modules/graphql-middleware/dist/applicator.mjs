...
import { isMiddlewareFunction, isGraphQLObjectType, isMiddlewareWithFragment, isMiddlewareResolver } from 'graphql-middleware/dist/utils.mjs';
...
function applyMiddlewareToType(type, options, middleware) {
  ...
  if (isMiddlewareFunction(middleware)) {
    const resolvers = Object.keys(fieldMap).reduce((resolvers2, fieldName) => {
      resolvers2[fieldName] = applyMiddlewareToField(fieldMap[fieldName], options, middleware);
      return resolvers2;
    }, {});
    return resolvers;
  } else {
    const resolvers = Object.keys(middleware).reduce((resolvers2, fieldName) => {
      resolvers2[fieldName] = applyMiddlewareToField(fieldMap[fieldName], options, middleware[fieldName]);
      return resolvers2;
    }, {});
    return resolvers;
  }
...
function applyMiddlewareToSchema(schema, options, middleware) {
  ...
  const resolvers = Object.keys(typeMap).filter((type) => isGraphQLObjectType(typeMap[type]) && !isIntrospectionType(typeMap[type])).reduce((resolvers2, type) => {
    resolvers2[type] = applyMiddlewareToType(typeMap[type], options, middleware);
    return resolvers2;
  }, {});
  return resolvers;
}
function generateResolverFromSchemaAndMiddleware(schema, options, middleware) {
  if (isMiddlewareFunction(middleware)) {
    ...
    const resolvers = Object.keys(middleware).reduce((resolvers2, type) => {
      resolvers2[type] = applyMiddlewareToType(typeMap[type], options, middleware[type]);
      return resolvers2;
    }, {});
    return resolvers;
  }
